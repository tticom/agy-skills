...
def write_workspace_index(workspace, repos, active_task_path):
    lines = [
        "# Workspace index",
        "",
        "Generated by the cleanup skill. Canonical source remains each checkout's Git state.",
        "",
        "| Checkout | Branch | HEAD | State |",
        "|---|---|---|---|",
    ]
    for repo in sorted(repos, key=lambda path: path.name):
        branch = run_cmd(["git", "branch", "--show-current"], cwd=repo).stdout.strip() or "detached"
        head = run_cmd(["git", "rev-parse", "--short", "HEAD"], cwd=repo).stdout.strip()
        state = "dirty" if is_dirty(repo) else "clean"
        lines.append(f"| `{repo.name}` | `{branch}` | `{head}` | {state} |")
    lines.extend(["", f"Active task pointer: `{active_task_path}`", ""])
    output = workspace / "agy-logs" / "workspace-state" / "latest.md"
...
def main():
    ...
    receipt.append({"action": "workspace_index", "path": str(index_file), "reason": "human-readable checkout map"})
